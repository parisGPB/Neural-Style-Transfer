...
##Añadido Marc libreria graficos
...
#--Codigo Marc
content_loss_type = args.content_loss_type
...
##Codigo Añadido Marc ------------------------
loss_vect = list()
improvement_vect = list()
timeexec_vect = list()
total_time = 0
##Codigo Añadido Marc------------------------
#for i in range(num_iter):
#num_iter = 2  Solo para test
for i in range(num_iter):
	...
	##Codigo Añadido Marc------------------------
    loss_vect.append(min_val)
    improvement_vect.append(improvement)
    if i == num_iter:
        print("Current loss value vector: ")
        print(loss_vect)
        print("Improvement vector: ")
        print(improvement_vect)
        print(".")
	##Codigo Añadido Marc------------------------
    ...
    nombredir = '/home/dlai_styletransfer/Neural-Style-Transfer/carpetatests/outputs/'
    fname = nombredir + '_at_iteration_%d.png' % (i + 1)
    ...
    if i < 10:
        imsave(fname, img)
    ##Codigo Añadido Marc------------------------ Solo guardamos las 10 primeras y después cada 10
    ...
    print(exec_time)
    timeexec_vect.append(exec_time)
    print("Execution time vector: ")
    print(exec_time)
	...
	##Codigo Añadido Marc------------------------
	## Guardando datos de graficas (tiempo, loss y mejora)
    #print("Antes de verificar iter")
    ...
    if i == num_iter-1:
        ...
        plt.title('Loss Value at iteration' + str(i))
        plt.savefig('/home/dlai_styletransfer/Neural-Style-Transfer/carpetatests/outputs/loss-result.png')

        plt.clf()

        plt.plot(improvement_vect)
        plt.xlabel('Iterations')
        plt.ylabel('Improvement Value (%)')
        plt.title('Improvement Value at iteration' + str(i))
        plt.savefig('/home/dlai_styletransfer/Neural-Style-Transfer/carpetatests/outputs/improvement-result.png')

        plt.clf()

        plt.plot(timeexec_vect)
        plt.xlabel('Iterations')
        plt.ylabel('Execution time (seconds)')
        plt.title('Execution time at iteration' + str(i))
        plt.savefig('/home/dlai_styletransfer/Neural-Style-Transfer/carpetatests/outputs/time-result.png')

        print("TEST DATA SAVED")
        print("Generating TXT file...")
        f= open("/home/dlai_styletransfer/Neural-Style-Transfer/carpetatests/outputs/outputInfo.txt","w+")
        f.write("Num_iter:  "+ str(num_iter))
        f.write('\n')
        f.write("Model:   " + str(model))
        f.write('\n')
        f.write("content_weight : "+ str(content_weight) )
        f.write('\n')
        f.write(" style_weights: "+str(style_weights))
        f.write('\n')
        f.write("Total_variation_weight  : " + str(total_variation_weight))
        f.write('\n')
        f.write("Image Size:  " + str(img_width) +"x"+ str(img_height))
        f.write('\n')
        f.write("Content Loss Type:  " + str(content_loss_type))
        f.write('\n')
        f.write("Content Layer:  " + str(content_layer))
        f.write('\n')
        f.write("Pool Type:  " + str(pool_type))
        f.write('\n')
        f.write("Feature Layers:  " + str(feature_layers))
        f.write('\n')
        f.write("Init image:  " + str(init_image))
        f.write('\n')
        f.close()
